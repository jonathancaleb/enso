...
import com.example.enso.data.sms.RawSms
...
class MoMoSmsParserTest {
    ...
    fun `parse withdrawal`() {
        ...
        val result = MomoSmsParser.parse(raw(body))
    ...
    fun `parse deposit`() {
        ...
        assertEquals("Received - SOPHIE CHELIMO", result.description)
    }
    ...
    fun `parse transfer`() {
        ...
        assertEquals("40265187290", result.transactionId)
        assertEquals("Sent to 256750681731, SOPHIE,CHELIMO", result.description)
    }
    ...
    private fun raw(body: String, sender: String = "MTNMobMoney"): RawSms {
